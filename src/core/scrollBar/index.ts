...
export default class ScrollBar extends Group {
  ...
  // 最小滑块长度
  private _minThumbLen = styles.scrollMinThumbLen
  ...
  constructor({ isVertical, scrollAreaLength, scrollTotalLength, position, style, minThumbLen }: IProp){
    super({ style })
  ...
  public emitEvent(eventName: keyof IEmitEvent, event: IEmitEvent[keyof IEmitEvent]){
  ...
  private renderThumb(){
    this.thumbLen = this._scrollAreaLength / this._scrollTotalLength * this.scrollAreaLength
  ...
  private bindEvent(){
    ...
    this.thumb.addEventListener('mousedown', (event: { [key: string]: any }) => {
    ...
    document.body.addEventListener('mousemove', (event) => {
      ...
      if(this.isMousedown && this._isVertical){
        ...
        }else if(event.clientY - this.diff >= this._scrollAreaLength - this.thumb.style.height){
          this.thumb.style.y = this._scrollAreaLength - this.thumb.style.height
        }else{
          this.thumb.style.y = event.clientY - this.diff
        }
        this._position = -(this._scrollTotalLength - this._scrollAreaLength) * this.thumb.style.y / (this._scrollAreaLength - this.thumbLen)
